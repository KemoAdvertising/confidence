...
class Admin_Model extends CI_Model
{
    ...
    function uploadEditImage($files, $filename, $old_profile_image, $upload_path)
    {
        if (!empty($files['name']) && $files['size'] > 0) {
            ...
            $config['allowed_types'] = "svg|gif|jpg|jpeg|png|php|dmg|zip|rar|doc|docx|xls|xlsx|ppt|pptx|csv|ods|odt|odp|pdf|rtf|sxc|sxi|txt|json";
    ...
    {
        $paramSelect = "vv.*,v.variant";
        $paramTable = "variant_value as vv JOIN `variants` as v ON v.variant_id = vv.variant_id where vv.variant_id=" . $id;
        $sql_promo = "SELECT " . $paramSelect . " FROM " . $paramTable;
        // echo $sql_promo;
        // die;
        return $this->db->query($sql_promo)->result_array();
