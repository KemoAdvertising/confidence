...
{
 $query = $this->db->get("variant_value");
        return $query->result();
}
    ...
    function getPriceDetails1($str = "")
    {
        // $objectStr = isset($str) && $str != "" ? " AND " . $str . " GROUP BY pv.sku" : '';

        $sql_promo = "SELECT p.product_Variants_id, " . $str . "FROM product_variants p 
        JOIN product_details po ON po.product_Variants_id = p.product_Variants_id
        GROUP BY p.product_Variants_id";
        // $sql_promo = "SELECT * FROM products p inner join  product_variants pv on p.product_id = pv.product_id where pv.product_id = 1";
        // echo $sql_promo;

        return $this->db->query($sql_promo)->result_array();
    }
    function getProductVariant($abc = '')
    {
        $str = $abc;
        if (!$abc) {
            $str = 1;
        ...
        inner join product_details pd on pd.product_Variants_id = pv.product_Variants_id
        inner join variant_value vv on vv.value_id = pd.value_id
        inner join variants v on v.variant_id = vv.variant_id where pv.product_id = 1 AND pv.product_Variants_id=" . $str;
        // $sql_promo = "SELECT * FROM products p inner join  product_variants pv on p.product_id = pv.product_id where pv.product_id = 1";
        // echo $sql_promo;
