...
class User extends \Restserver\Libraries\REST_Controller
{
    ...
    function productPrice_post()
    {
        ...
        if (!empty($post_Array_data) && isset($post_Array_data['option'])) {
            ...
            foreach ($post_Array_data['option'] as $key => $item) {
                $orStr = $key < count($post_Array_data['option']) - 1 ? "," : '';
                // $optionStr .=  $orStr . ' pd.value_id=' .  $item;
                $optionStr .= 'COALESCE(MAX(CASE WHEN po.value_id = ' . $item . ' THEN po.value_id END),0) AS `Option ' . $key . '`' . $orStr;
            }
            // print_r($optionStr);
            // die;
            $variants = $this->UserModel->getVariants();
            ...
            if (!empty($optionStr)) {
                $data = $this->UserModel->getPriceDetails1($optionStr);
            } else {
                $data = $skuData;
            }
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $d => $val) {
                    ...
                    if ($flg == 0) {
                        $abc = $val['product_Variants_id'];
                    }
                }
                // $dataOption = $data;
            }
            $dataOption = $this->UserModel->getProductVariant($abc);
                ...
                foreach ($skuData as $key2 => $value) {
                    ...
                    if ($var['variant_id'] == $value['variant_id']) {
                        ...
                        if (!in_array($value['value_id'], $dataArrayOption)) {
                            ...
                            $dataArray[$var['variant_id']]['options'][$optionCount]['text'] = $value['value'];
                            $dataArray[$var['variant_id']]['options'][$optionCount]['valuedata'] = $value['value_data'];
                            $optionCount++;
                        }
                    }
                }
            ...
            if(!empty($dataOption)){
                $dataArray['sku'] = $dataOption[0]['sku'];
                $dataArray['product_Variants_id'] = $dataOption[0]['product_Variants_id'];
                $dataArray['productVariantName'] = $dataOption[0]['productVariantName'];
                $dataArray['price'] = $dataOption[0]['price'];
                // $dataArray[$var['variant_id']]['productVariantImage'] = $data[0]['productVariantImage'];
                $dataArray['productVariantImage'] = base_url('assets/uploads/variations/' . $dataOption[0]['image']);
                $message = ['status' => TRUE, 'message' => 'productdetails', 'data' => $dataArray];
   ...
   public function getVariantsName()
{
     $employee =$this->App_model->getVariantsName();
 if ($employee) {
            $message = ['status' => False, 'message' => $employee];
            $httpResponse = REST_Controller::HTTP_CREATED;
        } else {
            $message = ['status' => TRUE, 'message' => 'Order Submitted succesfully'];
            $httpResponse = REST_Controller::HTTP_CREATED;
